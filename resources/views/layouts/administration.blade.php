...
<body class="vdbs-app-shell">
    ...
    <a class="vdbs-skip-link" href="#administration-content">
        Zum Inhalt
    </a>
    ...
    <main id="administration-content" class="site-main vdbs-public-main">
        <x-vdbs.frame width="normal" gutter="both">
            @if (session('status'))
                @php
                    $administrationStatusType = session(
                        'status_type',
                        'info',
                    );
                    $administrationStatusRole = in_array(
                        $administrationStatusType,
                        ['danger', 'warning'],
                        true,
                    )
                        ? 'alert'
                ...
                <x-vdbs.notice
                    :type="$administrationStatusType"
                    :role="$administrationStatusRole"
                >
                    {{ session('status') }}
                </x-vdbs.notice>
            @endif
