...
use App\Modules\Membership\Actions\SynchronizeMembershipRoleAction;
use App\Modules\Membership\Models\Membership;

function makeMembershipManagementActor(
    RoleKey $roleKey,
    string $email,
): User {
...
it('allows board members to read and manage membership data', function () {
    $board = makeMembershipManagementActor(
        RoleKey::BoardMember,
        'membership-board@example.test',
    );
    $person = makeMembershipPerson(
        'membership-person-board@example.test',
    );
    $membership = Membership::query()->create([
        'person_id' => $person->id,
        'starts_on' => now()->subMonth()->toDateString(),
    ]);
    $newPerson = makeMembershipPerson('membership-new-board@example.test');
    ...
    $newMembership = Membership::query()->where('person_id', $newPerson->id)->firstOrFail();
    $response->assertRedirect(route('administration.memberships.show', $newMembership));
...
it('blocks membership administration for users without administration access', function () {
    $user = User::query()->create([
        'email' => 'membership-no-access@example.test',
        'password' => 'Sicher123!',
        'status' => UserStatus::Active,
        'session_version' => 1,
    ]);
...
it('creates a membership, audit event and synchronized member role', function () {
    $admin = makeMembershipManagementActor(
        ...
        'membership-admin-create@example.test',
    );
    ...
    expect($assignment->source)
        ...
        ->and($assignment->starts_at->toDateString())
        ->toBe($startsOn)
        ->and($assignment->ends_at)
        ->toBeNull();

    $audit = AuditEvent::query()
        ->where('event_key', AuditEventCatalog::MEMBERSHIP_CREATED)
        ->where('subject_id', $membership->id)
        ->firstOrFail();

    expect($audit->actor_user_id)
        ->toBe($admin->id)
        ->and($audit->new_values['person_id'] ?? null)
        ->toBe($person->id);
});
...
it('creates memberships for persons without portal accounts without role assignments', function () {
    $admin = makeMembershipManagementActor(
        RoleKey::Administration,
        'membership-admin-no-user@example.test',
    );
...
it('rejects overlapping membership periods for the same person', function () {
    $admin = makeMembershipManagementActor(
        RoleKey::Administration,
        'membership-admin-overlap@example.test',
    );
...
it('updates membership dates and keeps the automatic member role synchronized', function () {
    $admin = makeMembershipManagementActor(
        RoleKey::Administration,
        'membership-admin-update@example.test',
    );
    ...
    app(SynchronizeMembershipRoleAction::class)->execute($membership);
    ...
    expect($membership->starts_on->toDateString())
        ->toBe($newStart)
        ->and($membership->ends_on?->toDateString())
        ->toBe($newEnd);
    ...
    expect($assignment->starts_at->toDateString())
        ->toBe($newStart)
        ->and($assignment->ends_at?->toDateString())
        ->toBe($newEnd);

    expect(
        AuditEvent::query()
            ->where('event_key', AuditEventCatalog::MEMBERSHIP_UPDATED)
            ->where('subject_id', $membership->id)
            ->exists(),
    )->toBeTrue();
});
...
it('ends an open membership with a required reason and audits it', function () {
    $admin = makeMembershipManagementActor(
        RoleKey::Administration,
        'membership-admin-end@example.test',
    );
    ...
    $membership->refresh();
    expect($membership->ends_on?->toDateString())
    ...
    expect($assignment->ends_at?->toDateString())
        ->toBe(now()->toDateString());

    $audit = AuditEvent::query()
        ->where('event_key', AuditEventCatalog::MEMBERSHIP_ENDED)
        ->where('subject_id', $membership->id)
        ->firstOrFail();
...
it('requires an end date and reason when ending a membership', function () {
    $admin = makeMembershipManagementActor(
        RoleKey::Administration,
        'membership-admin-end-validation@example.test',
    );
    $person = makeMembershipPerson(
        'membership-end-validation@example.test',
    );
    $membership = Membership::query()->create([
        'person_id' => $person->id,
        'starts_on' => now()->subMonth()->toDateString(),
    ...
    $this
        ...
        ->assertSessionHasErrors(['ends_on', 'reason']);
...
it('does not reopen ended memberships and supports resumption as a new period', function () {
    $admin = makeMembershipManagementActor(
        RoleKey::Administration,
        'membership-admin-resume@example.test',
    );
    ...
    expect(
        ...
        ->toBe(2)
        ->and($membership->refresh()->ends_on?->toDateString())
        ->toBe($oldEnd);
});
...
it('filters memberships by person and derived status', function () {
    $board = makeMembershipManagementActor(
        RoleKey::BoardMember,
        'membership-filter@example.test',
    );
...
it('shows membership history to administration with an additional board role and paginates memberships for board members', function () {
    ...
    $board = makeMembershipManagementActor(
        RoleKey::BoardMember,
        'membership-pagination-board@example.test',
    );
    ...
    foreach (range(1, 24) as $index) {
        $person = makeMembershipPerson(
            sprintf('membership-page-%02d@example.test', $index),
        );
        Membership::query()->create([
            'person_id' => $person->id,
            'starts_on' => now()->subDays($index)->toDateString(),
        ]);
    }
    ...
    $this
        ->withSession(membershipManagementSession())
        ->actingAs($admin)
        ->get('http://my.vdb.test/verwaltung/personen/'.$firstPerson->id)
        ->assertOk()
        ->assertSee('Mitgliedschaftsverlauf')
        ->assertSee('2')
        ->assertSee('Mitgliedschaft anlegen');
